...
class UserService
{
    ...
    public function validator(Request $request, $id = null): Validator
    {
        $rules = [
            ...
            'office_id' => [
                //'nullable|requiredIf:user_type_id,' . UserType::USER_TYPE_AC_LAND_USER_CODE . ',' . UserType::USER_TYPE_UNO_USER_CODE . ',' . UserType::USER_TYPE_TRAINING_CENTER_USER_CODE . ',' . UserType::USER_TYPE_TRAINER_USER_CODE,
                'nullable',
                'int',
                'exists:offices,id'
            ],
        ...
        if(!empty($request->user_type_id) && ((($request->user_type_id == 1 || $request->user_type_id == 2)))){
            $rules['office_id'] = 'nullable|int|exists:offices,id';
        }else{
            $rules['office_id'] = 'required|int|exists:offices,id';
        }
    ...
    public function getListDataForDatatable(Request $request): JsonResponse
    {
        ...
        $users->leftJoin('offices', 'users.office_id', '=', 'offices.id');
