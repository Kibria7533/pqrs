    ...
    <div class="row">
        <div class="col-md-12">
            <div id="PrintArea" style="margin-left: 20px">
                <?php
                // $designations = Module\Khatian\Models\MutedKhotian::SIGNATURE_DESIGNATION;
                function eng_to_bangla_code_renew($input){
                ...
                $per_page_line_number = 22;  //22 was main value
...
//        $line = preg_replace('/\s+/', '&nbsp;', $line);
            ...
            {{-- TODO: start here arman --}}
                ...
                foreach ($owners as $key => $owner) {
                    ...
                    $address_entry = false;
               /*      for ($i = 0; $i < $owner['address_line_num']; $i++) { // this actually set line break
                        $pre_owner_address_break[$key + 1] = isset($pre_owner_address_break[$key + 1]) ? $pre_owner_address_break[$key + 1] . '<br>' :  '<br>';
                    } */
                    ...
                    if ($pre_address_only !== false) {
                        if ($pre_parent_name != $owner['parent_name']) {
                            if($pre_address_only !=  $owner['address_only']){
                               ...
                               /*  $owner_data .= ' <br>' . $owner['parent_name'];
                                ...
                                // angsho break
                                $angshoBreak = 1 +  getLines($addressTMP, 34, '<br>', false);
                                // TODO:added  angshoBreakTmp here (arman)
                                $angshoBreakTmp[$key]=$angshoBreak;
                                ...
                                // angsho bresk
                                // $angshoBreak = 1;
                                $angshoBreak =  getLines($owner['owner_name'], 34, '<br>', false);
                                $addressTMP_new = preg_replace('#(\s*<br\s*/?>)*\s*$#i', '', $owner['address_only']);
                                $total_owner_data=$owner['owner_name'].'<br>'.$addressTMP_new;
                                $newAngsho= getLines($total_owner_data, 34, '<br>', false);

                                // dump([$angshoBreakTmp[$key-1],$key,$owner,$total_owner_data,$newAngsho]);
                                $angshoBreakTmp[$key-1]=$angshoBreak;
                                $angshoBreakTmp[$key]=$newAngsho;
                        ...
                        // TODO:added  angshoBreak here (arman)
                        $angshoBreak = 1 +  getLines($addressTMP, 34, '<br>', false);
                        $angshoBreakTmp[$key]=$angshoBreak;
                        // dump([$key]);
                        $pre_address_only = $owner['address_only'];
                        ...
                        // $pre_owner_address = $owner['owner_address'];
                        $pre_owner_address_holder = '';//$owner['owner_address'];
                        $pre_owner_data = $owner;
                    }
                    // mokodoma dhara son
                    ...
                    if ($address_entry && isset($angshoBreakTmp[$key-1])) {
                        // $owner_ongsho .=  $lineBreak[2] . '<span class="paddTwo" >' . $owner_area .'</span>'.' <br> <br>'.$lineBreak[($ownerCountLine-1)];  // worked a bit
                        // dump([$key,$angshoBreak,$angshoBreakTmp[$key-1],$ownerCountLine-1]);
                        $owner_ongsho .=  $lineBreak[$angshoBreakTmp[$key-1]] . '<span class="paddTwo">' . $owner_area .'</span>'.' <br>'.$lineBreak[($ownerCountLine - 1)];
                    } else {
                      
                        // while have same parent. so only name break allowed
                        // TODO: changed here for ongsho breaklines (arman)
                        // main code $owner_ongsho .= $owner_area .  $lineBreak[($ownerCountLine-1)].' <br>';
                        // dump([$key,$angshoBreak,$angshoBreakTmp,$ownerCountLine]);
                        $owner_ongsho .= $owner_area .  $lineBreak[($ownerCountLine-1)].' <br>';

                    //    $owner_ongsho .= $owner_area.'<br> <br>';// (arman)
                ...
                // Arrange the data of Khotian(arman)
                // added arman
                $jl_number='';
                $dcr_number='';
                $resa_no='';
                ...
                foreach ($khotian as $rowCnt => $row) {
                    ...
                    if(!empty($dagParts[1])){
                        // TODO: added bn2en in dags
                        $partOne = '<span class="dagUnderline">' . substr(trim(bn2en($dagParts[1])), 0, 5).  '</span> <br>';
                        $partTwo =  substr(trim(bn2en($dagParts[0])), 0, 5).' <br>';

                        $dag_no .= $partOne.$partTwo;
                        $dagLine = 2; //wordwrap($row['dag_number'], 5, "<br>", true);
                    }else{
                         // TODO: added bn2en in dags
                        $dag_no .= substr(bn2en(trim($row['dag_number'])), 0, 5). ' <br>';
...
//        $dagLines =   count( $dagArr );
                    ...
                    // dager mote poriman
                    // main code $dag_area = explode('.', $row['total_dag_area']);
                    $dag_area = explode('.', bn2en($row['total_dag_area']));
                    ...
                    $dag_portion = explode('.', bn2en($row['khotian_dag_area'])); //TODO:added bn2en 
                ...
                $temp_total_khotian_dag_portion='';
                $temp_total_khotian_dag_portion=$total_khotian_dag_portion;
               ...
               // $owner_data_pages = $overAllPages; // array_chunk($owner_data_lines_array, $per_page_line_number);
                $owner_data_pages = array_chunk($overAllPages[0], $per_page_line_number);
                    ...
                    <table class="htable" border="0"  style="margin-top: 2px">
                        ...
                        <tr>
                            <td colspan="2" style="border:1px; font-size: 16px; padding-bottom: 15px;">
                                বিভাগঃ <?php echo $header_info['division_name']; ?></td>
